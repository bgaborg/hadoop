  ...
  public void put(PathMetadata meta) throws IOException {
    ...
    synchronized (this) {
      ...
      if (parentPath != null) {
        ...
        // Create empty parent LocalMetadataEntry if not exists
        if (parentMeta == null){
          parentMeta = new LocalMetadataEntry();
          localCache.put(parentPath, parentMeta);
        }

        // If there is no directory metadata is on the parent entry, create
        // an empty one
        ...
        // Set the listing entry deleted if the meta is set to deleted
        if(meta.isDeleted()) {
