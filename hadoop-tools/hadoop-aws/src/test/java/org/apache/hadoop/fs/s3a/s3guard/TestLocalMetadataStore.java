...
public class TestLocalMetadataStore extends MetadataStoreTestBase {
  ...
  public void testCacheTimedEvictionAfterWrite(){
    ...
    Cache<Path, PathMetadata> cache = CacheBuilder.newBuilder()
        .expireAfterWrite(
          LocalMetadataStore.DEFAULT_CACHE_ENTRY_TTL_SECONDS,
          TimeUnit.NANOSECONDS/* nanoseconds in test to avoid conversions */)
        .ticker(testTicker)
        .build();
    ...
    testTicker.myTicker =
      LocalMetadataStore.DEFAULT_CACHE_ENTRY_TTL_SECONDS - 2;
    ...
    // for the first entry
    testTicker.myTicker =
        LocalMetadataStore.DEFAULT_CACHE_ENTRY_TTL_SECONDS + 1;
